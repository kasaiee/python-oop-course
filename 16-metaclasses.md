...
`__set_name__` را فصل قبل کامل یاد گرفتید؛ اینجا از زاویه‌ی متاکلاسی نگاهش کنید تا جایگاهِ تاریخی‌اش روشن شود — این متد در واقع **جایگزینِ یکی از پرکاربردترین دلایلِ نوشتنِ متاکلاس** شد:
...
این دقیقاً همان مکانیزمی است که به `dataclass` و ORMهای SQLAlchemy اجازه می‌دهد نامِ فیلدها را خودکار بفهمند. پیش از پایتون ۳.۶، برای همین کارِ ساده باید متاکلاس می‌نوشتید که بدنه‌ی کلاس را بگردد و نام‌ها را به فیلدها تزریق کند. درسِ بزرگ‌تر: زبان به‌مرور کاربردهای رایجِ متاکلاس را به ابزارهای ساده‌تر (`__set_name__`، `__init_subclass__`) واگذار کرده — متاکلاس برای باقی‌مانده‌ی موارد است.
...
**۱. «متاکلاس چیست، در یک جمله؟»** — «کلاسِ کلاس‌ها»: همان‌طور که کلاس، ساختنِ اشیاء را کنترل می‌کند، متاکلاس ساختنِ خودِ کلاس‌ها را کنترل می‌کند؛ پیش‌فرضش `type` است. اگر `type(name, bases, namespace)` را هم زنده بنویسید، جواب کامل است.

**۲. «کِی متاکلاس می‌نویسی؟»** — جوابِ درست با نقل‌قولِ تیم پیترز شروع می‌شود: تقریباً هیچ‌وقت. سلسله‌ی جایگزین‌ها: دکوراتورِ کلاس → `__init_subclass__` → `__set_name__` → و فقط اگر باید *خودِ* فرایندِ ساختِ کلاس را عوض کنید (تغییرِ namespace قبل از ساخت، کنترلِ `__prepare__`، چیزی مثل ORM کامل)، متاکلاس. دانستنِ «چرا نه» اینجا مهم‌تر از «چطور» است.

**۳. «Django برای مدل‌هایش متاکلاس دارد؛ چرا؟»** — چون هنگامِ تعریفِ هر مدل، کارهایی فراتر از هوکِ ساده لازم دارد: جمع‌کردنِ فیلدها، ساختنِ `objects` (Manager) و استثناهای اختصاصی، ثبت در registryِ اپ‌ها — دست‌کاریِ کلاس **پیش و پس از** ساخت. اشاره به اینکه نسخه‌ی ساده‌اش با `__init_subclass__` هم شدنی است (و شما ساخته‌اید — پروژه‌ی چهارم!)، شما را از حفظ‌کارها جدا می‌کند.

---

**در فصل بعد:** از انتزاعِ عمیق به دنیای عمل برمی‌گردیم: **Performance، هم‌زمانی و async** — چطور کدِ شیءگرا را سریع، مقیاس‌پذیر و thread-safe نگه داریم.
