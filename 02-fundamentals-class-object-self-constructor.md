...
این فصل، پایه‌ی همه‌چیز است. هر مفهومی که در فصل‌های بعد می‌آید — تفکر شیءگرا، ارث‌بری، Polymorphism، متاکلاس — روی چهار مفهوم همین فصل ساخته می‌شود: **کلاس، شیء، `self` و سازنده**. اگر این چهار را عمیق بفهمید، بقیه‌ی دوره روان پیش می‌رود. اگر سطحی رد شوید، بعدها به باگ‌های گیج‌کننده برمی‌خورید که ریشه‌شان همین‌جاست.
...
بیشتر باگ‌های برنامه‌نویسان تازه‌کار در شیءگرایی، از یک‌جا می‌آید: درک نادرست از اینکه داده **کجا** زندگی می‌کند و `self` **به چه** اشاره می‌کند. یک نمونه‌ی کلاسیک:
...
اگر نمی‌دانید خروجی خط آخر چیست و چرا، این فصل دقیقاً برای شماست. (پاسخش را در بخش Class/Instance Variable می‌بینید.) درک درست این مفاهیم به شما کمک می‌کند:
...
**شیء** (یا نمونه / instance) یک ساخت واقعی از آن قالب در حافظه است. از روی یک نقشه می‌توان صدها خانه ساخت؛ از روی یک کلاس می‌توان هزاران شیء ساخت که هرکدام داده‌ی خودش را دارد.
...
این تمایز، منشأ همان باگ ابتدای فصل است. دو نوع متغیر داریم:
