...
| هدف اولیه | شبیه‌سازی سیستم‌های واقعی | محیط محاسباتی زنده و تعاملی |
...
**پارادایم منطقی (Logic):** برنامه مجموعه‌ای از واقعیت‌ها و قواعد است و موتور زبان از آن‌ها نتیجه استنتاج می‌کند. Prolog نمونه‌ی اصلی است و بیشتر در هوش مصنوعی کلاسیک و سیستم‌های خبره کاربرد دارد تا نرم‌افزار روزمره.
...
فعلاً همین اندازه کافی است بدانید که چنین چیزی **وجود دارد** و پایتون آن را با الگوریتمی به نام **C3** به‌شکل منظم و قابل‌اتکا مدیریت می‌کند. نیازی نیست الان جزئیاتش را بفهمید — چون هنوز حتی ارث‌بری را یاد نگرفته‌ایم.

> MRO به‌طور طبیعی جای خودش را در بحث ارث‌بری دارد. الگوریتم C3، مسئله‌ی الماس (Diamond Problem)، و مثال‌های کامل را در [فصل ششم: ارث‌بری پیشرفته و MRO](06-advanced-inheritance-mro.md) — بعد از اینکه ارث‌بری را کامل یاد گرفتید — گام‌به‌گام می‌بینیم.

### شیءگرایی پایتون کمی با بقیه فرق دارد

شیءگرایی در هر زبان لهجه‌ی خودش را دارد. فعلاً فقط دو نکته را از پایتون به‌خاطر بسپارید:

- **در پایتون همه‌چیز شیء است** — حتی یک عدد، یک تابع، و حتی خود یک کلاس. این یکپارچگی، از میراث Smalltalk می‌آید.
- **کپسول‌سازی در پایتون قراردادی است، نه اجباری** — همان نقدی که کمی بالاتر دیدیم؛ پایتون `private` اجباری ندارد و به بلوغ برنامه‌نویس تکیه می‌کند.

و یک نکته‌ی جالب: **شبیه‌ترین زبان به پایتون، روبی است** — هر دو پویا، قراردادمحور و منعطف‌اند. (اگر از جاوا یا C++ می‌آیید، فقط این را بدانید که پایتون `private` اجباری ندارد و ارث‌بری چندگانه را هم مجاز می‌داند؛ عادت‌های آن زبان‌ها را کورکورانه به پایتون نیاورید.)
...
**۵. بازار کار آن را می‌خواهد.** کافی است چند آگهی استخدام پایتون را بخوانید؛ تقریباً همه‌شان تسلط بر OOP را شرط کرده‌اند. این تنها دلیل خوب یادگیری نیست، اما دلیل واقعی‌ای است.

## خلاصه

نقشه‌ی راه این فصل را در یک نگاه ببینید:
...
═══════════════════════════════════════════════
  ...
  C++ (۱۹۸۳)         ──►  شیءگرایی در صنعت
        │
        ▼
  Python 1.0 (۱۹۹۴)  ──►  شیءگرایی اولیه (با محدودیت)
        │
        ▼
...
| مسئله‌ای که حل کرد | سازمان‌دهی کد بزرگ و کنترل دسترسی به داده |
...
| تفاوت با جاوا | ارث‌بری چندگانه، کپسول‌سازی قراردادی، نوع‌بندی پویا |
| شبیه‌ترین زبان | Ruby |
...
سوال‌هایی از همین فصل که در مصاحبه‌ها می‌پرسند — قبل از خواندن پاسخ، خودتان جواب دهید.

**۱. «شیءگرایی را در یک جمله تعریف کنید.»** — جواب کلیشه‌ای («کلاس و ارث‌بری») نمره‌ی کامل نمی‌گیرد. جواب عمیق‌تر از آلن کی وام می‌گیرد: سازمان‌دهی برنامه به‌صورت موجودیت‌های مستقلی که داده و رفتارشان یک‌جاست و با پیام (فراخوانی متد) با هم کار می‌کنند. اشاره به اینکه «کلاس» فقط یکی از راه‌های رسیدن به این ایده است، شما را متمایز می‌کند.

**۲. «منتقدان شیءگرایی چه می‌گویند و چقدر حق دارند؟»** — سه نقد اصلی: مدیریت State پنهان (منشأ باگ‌های سخت)، شکنندگی سلسله‌مراتب وراثت، و کپسوله‌سازی ناقص در عمل. پاسخ بالغ نشان می‌دهد نقدها را می‌شناسید و راه‌حل‌های مدرن (ترکیب به‌جای وراثت، تغییرناپذیری) دقیقاً از دل همین نقدها آمده‌اند — نه اینکه از پارادایم دفاع تعصبی کنید.

**۳. «پایتون چندپارادایمی است یعنی چه؟ پس چرا می‌گویند همه‌چیزش شیء است؟»** — دو حرف متفاوت‌اند: در پایتون می‌توانید رویه‌ای، تابعی یا شیءگرا *بنویسید* (سبک کد)، اما زیر کاپوت همه‌چیز — عدد، تابع، حتی خود کلاس — شیء *است* (مدل داده‌ی زبان). این تمایز «سبک در برابر مدل زیرین» نشانه‌ی فهم واقعی است.
