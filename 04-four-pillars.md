...
اول، `Shape` از `ABC` ارث برده و `area` با `@abstractmethod` علامت خورده است؛ نتیجه اینکه پایتون **اجازه‌ی ساختن شیء مستقیم از `Shape` را نمی‌دهد** مگر اینکه هنگام ارث بری،‌متدی که در والد با `@abstractmethod` مشکل شده را حتما در فرزند override کنید. یک چیز انتزاعی در دنیای واقعی وجود ندارد — و حالا زبان هم این را ضمانت می‌کند.
