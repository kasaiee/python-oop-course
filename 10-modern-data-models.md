...
- `frozen=True`: شیء را تغییرناپذیر می‌کند (ایمن در برابر تغییرِ ناخواسته؛ مزایای عمیق‌ترِ Immutable را در فصل چهاردهم می‌بینیم).
...
یادِ دامِ «پیش‌فرضِ تغییرپذیر» از فصل دوم (لیستِ مشترکِ `Team.members`)؟ در dataclass نمی‌توانید مستقیم `items: list = []` بنویسید (پایتون خطا می‌دهد). به‌جایش از `default_factory` استفاده کنید:
...
**۱. «dataclass پشتِ صحنه چه می‌سازد و کِی استفاده‌اش نمی‌کنی؟»** — از روی annotationها، `__init__` و `__repr__` و `__eq__` (و با پارامتر: `__hash__`، متدهای ترتیب، `__slots__`) تولید می‌کند. استفاده نمی‌کنم وقتی کلاس عمدتاً **رفتار** است نه داده، یا مقداردهی‌اش منطقِ پیچیده دارد که در `__post_init__` هم زورکی جا می‌شود.

**۲. «فرق dataclass و Pydantic؟»** — dataclass فقط کد تولید می‌کند و به تایپ‌ها **اعتماد** دارد؛ Pydantic در زمانِ اجرا **اعتبارسنجی و تبدیل** می‌کند. مرزِ داخلیِ برنامه → dataclass (ارزان)؛ مرزِ با دنیای بیرون — ورودیِ API و فایلِ پیکربندی → Pydantic (بی‌اعتمادیِ موجه). جوابِ ممتاز همین «داخل/مرز» است.

**۳. «چرا `items: list = []` در dataclass خطا می‌دهد؟»** — همان دامِ پیش‌فرضِ تغییرپذیر: یک لیست برای همه‌ی نمونه‌ها. dataclass به‌جای اینکه بگذارد در production بترکید، همان لحظه‌ی تعریف جلویتان را می‌گیرد؛ راهِ درست `field(default_factory=list)` است — کارخانه‌ای که برای هر نمونه، ظرفِ تازه می‌سازد.

**۴. «Enum چه مشکلی را حل می‌کند که رشته حلش نمی‌کند؟»** — غلطِ تایپی (`"pade"` به‌جای `"paid"`) با رشته، باگِ خاموش است و با Enum، `AttributeError`ِ بلند؛ مجموعه‌ی مقادیر بسته و قابلِ‌کشف است (`list(Status)`)؛ و در تایپ‌هینت، `status: Status` واقعاً چیزی را تضمین می‌کند که `status: str` نمی‌کند.
