...
در این فصل، سه دسته ابزار قدرتمند پایتون را بررسی کردیم که کد شیءگرای شما را از سطح «درست» به سطح «حرفه‌ای» ارتقا می‌دهند. این ابزارها به شما اجازه می‌دهند تا کلاس‌های خود را با ساختارهای زبانی پایتون یکپارچه کنید و کنترل بیشتری بر نحوه‌ی دسترسی، محاسبه و نمایش داده‌ها داشته باشید.

### ۱. Property: ویژگی‌های پویا و هوشمند

`property` مکانیزمی است که به شما امکان می‌دهد یک ویژگی را به گونه‌ای تعریف کنید که مانند یک attribute ساده در دسترس باشد، اما در پشت صحنه، منطق دلخواه شما اجرا شود. این ابزار سه کاربرد اصلی دارد:

- **ویژگی‌های محاسبه‌شونده**: مانند `age` که از `birth_date` محاسبه می‌شود یا `area` که از `width` و `height` به‌دست می‌آید.
- **اعتبارسنجی و کنترل دسترسی**: مانند محدود کردن نمره‌ی دانشجو به بازه‌ی ۰ تا ۱۰۰.
- **برقراری ارتباط دوطرفه میان ویژگی‌ها**: مانند تنظیم `age` که به‌روزرسانی `birth_date` را در پی دارد.

با `@cached_property` نیز می‌توان محاسبات سنگین را فقط یک بار انجام داد و نتیجه را کش کرد. قاعده‌ی طلایی این است که با attribute ساده شروع کنید و فقط در صورت نیاز به منطق، آن را به `property` تبدیل کنید. این رویکرد، کد کاربر را نمی‌شکند و انعطاف‌پذیری بالایی فراهم می‌آورد.

### ۲. دکوراتورهای کلاسی: classmethod و staticmethod

پایتون دو نوع متد خاص برای کار با کلاس و داده‌های سطح کلاس ارائه می‌دهد:

- **`@classmethod`**: متدی که اولین پارامتر آن `cls` (خود کلاس) است. کاربرد اصلی آن به عنوان **Factory Method** برای ساخت نمونه با روش‌های جایگزین (مانند `Date.from_string()`) است. همچنین برای دسترسی یا تغییر متغیرهای کلاس به کار می‌رود. مزیت استفاده از `cls` به جای نام کلاس، حفظ انعطاف‌پذیری در ارث‌بری است.

- **`@staticmethod`**: متدی که نه `self` دریافت می‌کند و نه `cls`؛ صرفاً یک تابع کمکی است که در فضای نام کلاس قرار می‌گیرد. زمانی استفاده می‌شود که تابعی منطقاً به کلاس مرتبط است اما به داده‌های نمونه یا کلاس نیازی ندارد (مانند توابع اعتبارسنجی).

### ۳. متدهای جادویی (Dunder Methods)

متدهای جادویی، قلاب‌هایی هستند که پایتون در موقعیت‌های خاص خودکار صدا می‌زند. با پیاده‌سازی آنها، کلاس شما با عملگرها، توابع داخلی و ساختارهای زبانی یکپارچه می‌شود. مهم‌ترین آنها عبارتند از:

| دسته‌بندی | متدهای جادویی | کاربرد |
|-----------|---------------|---------|
| **نمایش** | `__str__`, `__repr__`, `__format__` | نمایش شیء برای کاربر و برنامه‌نویس، کنترل قالب‌بندی |
| **برابری و هش** | `__eq__`, `__hash__` | تعیین برابری بر اساس مقدار، قابلیت استفاده در `set`/`dict` |
| **عملگرهای حسابی** | `__add__`, `__radd__`, `__mul__`, ... | پشتیبانی از عملگرهای `+`, `*` و غیره |
| **شرط‌ها** | `__bool__` | تعیین صحت‌وسقم در `if` و `while` |
| **کانتینرها** | `__getitem__`, `__setitem__`, `__delitem__`, `__len__`, `__contains__` | ایندکس‌گذاری، طول، عضویت (`in`) |
| **پیمایش** | `__iter__`, `__next__` | پیمایش در حلقه‌ی `for` (میان‌بر: جنریتور با `yield`) |
| **مدیریت زمینه** | `__enter__`, `__exit__` | پشتیبانی از عبارت `with` برای مدیریت منابع |
| **فراخوانی** | `__call__` | تبدیل شیء به یک تابع قابل فراخوانی |
| **مدیریت پویای attribute** | `__getattr__`, `__setattr__` | کنترل دسترسی به attributeهای گم‌شده یا اعتبارسنجی در زمان مقداردهی |
| **پیشرفته** | `__new__`, `__slots__` | کنترل فرآیند ساخت شیء و بهینه‌سازی حافظه |

### قاعده‌ی نهایی

متدهای جادویی و دکوراتورهای کلاسی ابزارهایی بسیار قدرتمند هستند، اما **همه‌ی آنها در همه‌ی کلاس‌ها لازم نیستند**. با ساده‌ترین شکل ممکن شروع کنید: attributeهای ساده، متدهای نمونه‌ی معمولی. فقط در صورتی که به منطق اضافی نیاز داشتید، از `property`، `classmethod`، `staticmethod` یا متدهای جادویی استفاده کنید. این رویکرد، کد شما را خوانا، قابل‌نگهداری و در عین حال قدرتمند نگه می‌دارد.

**به خاطر داشته باشید**: متدهای جادویی پلی هستند میان کلاس شما و خود زبان پایتون. از آنها برای طبیعی‌تر کردن تعامل با اشیاء خود استفاده کنید، نه برای نمایش قدرت.
