...
t = Temperature(25)
print(t.get_celsius())    # 25 — پرحرف و غیرپایتونی
t.set_celsius(30)
```
...
class Temperature:
    ...
    @property
    def celsius(self):               # مثل attribute خوانده می‌شود
        return self._celsius

    @celsius.setter
    def celsius(self, value):        # مثل attribute مقداردهی می‌شود
        if value < -273.15:
            raise ValueError("دما زیر صفر مطلق ممکن نیست")
        self._celsius = value

t = Temperature(25)
print(t.celsius)      # 25 — بدون پرانتز! مثل attribute
t.celsius = 30        # setter خودکار صدا زده می‌شود
# t.celsius = -300    # ValueError — اعتبارسنجی کار می‌کند
```
...
class Temperature:
    def __init__(self, celsius):
        self.celsius = celsius        # attribute ساده

# کدِ کاربر همه‌جا:  t.celsius = 30
```
...
class Circle:
    ...
    @property
    def area(self):                  # فقط‌خواندنی، محاسبه‌شونده
        return 3.14159 * self.radius ** 2

c = Circle(5)
print(c.area)      # 78.53975 — همیشه به‌روز است
c.radius = 10
print(c.area)      # 314.159 — خودکار بازمحاسبه شد
...
class Dataset:
    ...
    @cached_property
    def records(self):
        print("در حال خواندن فایل بزرگ...")   # فقط یک‌بار اجرا می‌شود
        # فرض کنید این عملیات سنگین است
        return [1, 2, 3]

d = Dataset("data.csv")
print(d.records)   # در حال خواندن فایل بزرگ... \n [1, 2, 3]
print(d.records)   # [1, 2, 3] — بار دوم فایل خوانده نشد، از کش آمد
```
...
class Example:
    ...
    def instance_method(self):       # به شیء دسترسی دارد (self)
        return self

    @classmethod
    def class_method(cls):           # به کلاس دسترسی دارد (cls)، نه شیء خاص
        return cls

    @staticmethod
    def static_method():             # نه self، نه cls — فقط یک تابع در فضای کلاس
        return "مستقل"
```
...
class Date:
    ...
    @classmethod
    def from_string(cls, text):          # Factory: از رشته بساز
        year, month, day = map(int, text.split("-"))
        return cls(year, month, day)     # cls یعنی Date (یا زیرکلاسش)

    @classmethod
    def today(cls):                      # Factory: تاریخ امروز
        import datetime
...
d1 = Date(2026, 7, 1)                    # سازنده‌ی معمولی
d2 = Date.from_string("2026-07-01")      # Factory از رشته
d3 = Date.today()                        # Factory از امروز
...
# بدون ساختن شیء قابل استفاده است
...
m = Money(1500000)
print(f"{m:long}")   # 1,500,000 تومان
print(f"{m}")        # 1500000
```
...
a = Point(1, 2)
b = Point(1, 2)
print(a == b)      # True — به‌خاطر __eq__ (مقدار یکسان)
print(a is b)      # False — دو شیء جدا در حافظه
print(a is a)      # True
```
...
print(len(points))   # 1 — چون برابرند و hash یکسان دارند
```
...
cart = Cart()
if not cart:                     # __bool__ صدا زده می‌شود
    print("سبد خالی است")       # چاپ می‌شود
```
...
print(p[0])                # آهنگ ۱     ← __getitem__
print("آهنگ ۲" in p)       # True      ← __contains__
...
class Countdown:
    ...
    def __iter__(self):
        return self                  # خودِ شیء iterator است

    def __next__(self):
        if self.current <= 0:
            raise StopIteration      # پایان پیمایش
        self.current -= 1
...
class DatabaseConnection:
    def __enter__(self):
        print("اتصال باز شد")
        return self                  # چیزی که به as داده می‌شود

    def __exit__(self, exc_type, exc_val, exc_tb):
        print("اتصال بسته شد")       # حتی اگر خطا رخ دهد اجرا می‌شود
        return False                 # False یعنی استثنا را سرکوب نکن

    def query(self, sql):
        return f"نتیجه‌ی: {sql}"

with DatabaseConnection() as db:
    print(db.query("SELECT * FROM users"))
# خروجی:
# اتصال باز شد
# نتیجه‌ی: SELECT * FROM users
# اتصال بسته شد   ← تضمین‌شده، حتی با خطا
```
...
print(limit())   # مجاز
print(limit())   # مجاز
print(limit())   # مسدود شد — شیء، حالتش را بین فراخوانی‌ها نگه داشت
```
...
class SafeConfig:
    def __getattr__(self, name):
        # فقط برای attribute های ناموجود
        return f"<{name} تعریف نشده>"

    def __setattr__(self, name, value):
        # دام حلقه‌ی بی‌نهایت! نباید self.name = value نوشت
        # چون خودش دوباره __setattr__ را صدا می‌زند
        super().__setattr__(name, value)   # راه درست

c = SafeConfig()
c.host = "localhost"
print(c.host)      # localhost
print(c.missing)   # <missing تعریف نشده>  ← __getattr__
```
...
class Product:
    price = Positive()      # منطق اعتبارسنجی، قابل استفاده‌ی مجدد
    weight = Positive()
...
p = Product(100, 5)
# Product(-1, 5)   # ValueError: باید مثبت باشد
```
