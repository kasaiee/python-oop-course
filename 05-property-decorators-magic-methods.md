...
در کلاس‌های پایتون، متدها به سه دسته تقسیم می‌شوند که هر یک نقش و کاربرد مشخصی دارند. در بخش‌های پیشین، با متدهای معمولی آشنا شدید که اولین پارامتر آنها `self` است و به یک نمونه‌ی خاص از کلاس دسترسی دارند. اکنون دو نوع دیگر را بررسی می‌کنیم که رابطه‌ی متفاوتی با کلاس و نمونه‌های آن برقرار می‌کنند.

### نگاهی به ساختار کلی

پیش از پرداختن به جزئیات، اجازه دهید یک نمای کلی از هر سه نوع متد را مشاهده کنید:
...
class Example:
    ...
    def instance_method(self):
        """دسترسی به نمونه (self) و داده‌های آن"""
        return self

    @classmethod
    def class_method(cls):
        """دسترسی به کلاس (cls) نه به نمونه‌ی خاص"""
        return cls

    @staticmethod
    def static_method():
        """دسترسی به هیچ‌کدام؛ صرفاً یک تابع در فضای نام کلاس"""
        return "independent"
```
...
### متد کلاس (classmethod) و کاربرد آن به عنوان Factory Method

یک متد کلاس با دکوراتور `@classmethod` مشخص می‌شود و اولین پارامتر آن به جای `self`، `cls` نام دارد. این پارامتر ارجاعی به خود کلاس است، نه به یک نمونه‌ی خاص از آن کلاس.
...
class Date:
    ...
    def from_string(cls, text):
        """ساخت شیء از یک رشته مانند '2026-07-21'"""
        year, month, day = map(int, text.split("-"))
        return cls(year, month, day)

    @classmethod
    def today(cls):
        """ساخت شیء با تاریخ امروز"""
        from datetime import date
        n = date.today()
...
```python
d1 = Date(2026, 7, 21)                     # روش معمول
d2 = Date.from_string("2026-07-21")        # از رشته
d3 = Date.today()                          # تاریخ امروز
...
class PersianDate(Date):
    """زیرکلاسی از Date برای تقویم هجری شمسی"""
    pass

# اگر از cls استفاده کرده باشیم، این متد به درستی یک شیء از نوع PersianDate می‌سازد
p = PersianDate.from_string("1405-04-30")
...
### متد ایستا (staticmethod) و کاربرد آن به عنوان تابع کمکی

یک متد ایستا با دکوراتور `@staticmethod` مشخص می‌شود و هیچ پارامتر خاصی (نه `self` و نه `cls`) دریافت نمی‌کند. این متدها صرفاً توابعی هستند که در فضای نام کلاس قرار گرفته‌اند.
...
class Validator:
    ...
    def is_valid_email(text):
        """بررسی اعتبار یک آدرس ایمیل"""
        return "@" in text and "." in text

    @staticmethod
    def is_valid_phone(text):
        """بررسی اعتبار یک شماره تلفن"""
...
- **خوانایی بیشتر**: کدخواهان می‌دانند که این تابع به چه مفهومی مربوط است.
...
| ارث‌بری | رفتار معمولی | با `cls` به‌درستی کار می‌کند | بدون تغییر می‌ماند |
...
```python
class User:
    # داده‌ی کلاس (مشترک بین همه‌ی نمونه‌ها)
    default_role = "guest"

    def __init__(self, username, email):
        # داده‌های نمونه (خاص هر شیء)
        self.username = username
        self.email = email

    # متد نمونه: به داده‌های شیء دسترسی دارد
    def display(self):
        return f"User: {self.username} ({self.email})"

    # متد کلاس: با کلاس کار می‌کند، نه با یک نمونه‌ی خاص
    @classmethod
    def from_dict(cls, data):
        """ساخت کاربر از دیکشنری"""
        return cls(data["username"], data["email"])

    @classmethod
    def get_default_role(cls):
        """دسترسی به داده‌ی کلاس"""
    ...
    # متد ایستا: تابع کمکی، به کلاس یا نمونه کاری ندارد
    @staticmethod
...
```python
# استفاده از متد نمونه
user = User("ahmad", "ahmad@site.com")
print(user.display())   # User: ahmad (ahmad@site.com)

# استفاده از متد کلاس (Factory)
new_user = User.from_dict({"username": "reza", "email": "reza@site.com"})
print(new_user.display())   # User: reza (reza@site.com)

# استفاده از متد کلاس برای دسترسی به داده‌ی کلاس
print(User.get_default_role())   # guest
User.set_default_role("admin")
print(User.get_default_role())   # admin

# استفاده از متد ایستا (بدون نیاز به نمونه)
print(User.is_valid_username("ahmad"))   # True
print(User.is_valid_username("a"))       # False
```

### نکته‌ی طلایی: چه زمانی از هر کدام استفاده کنیم؟

- **متد نمونه**: وقتی نیاز به کار با داده‌های یک نمونه‌ی خاص دارید. این رایج‌ترین نوع متد است.

- **متد کلاس**:
  - وقتی نیاز به ساخت نمونه با روش‌های جایگزین دارید (Factory Method).
  - وقتی نیاز به دسترسی یا تغییر داده‌های سطح کلاس دارید.
  - وقتی می‌خواهید متدی بنویسید که با ارث‌بری به‌درستی رفتار کند.

- **متد ایستا**:
  - وقتی تابعی منطقاً به کلاس مرتبط است اما به داده‌های کلاس یا نمونه نیاز ندارد.
  - وقتی تابع یک عملیات کمکی انجام می‌دهد که ارتباط تنگاتنگی با مفهوم کلاس دارد.

### جمع‌بندی: قدرت سازمان‌دهی
